...
$(() => {
    $.get(`https://5d76bf96515d1a0014085cf9.mockapi.io/product/${id}`, function(productList) {
        $('#product-image').append(`<img src = ${productList.preview}>`);
        ...
        for (let i of productList.photos) {
            $("#preview-image").append(`<img src = ${i} id = img${temp++} class = prev-img></img>`)
        }
        $(".prev-img:first").addClass("active");
        for( let i = 0; i<productList.photos.length;++i){
            $(".prev-img").eq(i).click(function(){
